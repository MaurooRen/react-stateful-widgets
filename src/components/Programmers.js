...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const found = listOfProgrammers.find(element => element.id === currentProgrammer)
    return found.name
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentProgrammer != null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfProgrammers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setCurrentProgrammer(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
